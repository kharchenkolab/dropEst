#include "RefGenesContainerWithIntons.h"
...
namespace Tools
{
	RefGenesContainerWithIntrons::RefGenesContainerWithIntrons()
		: _is_empty(true)
		, _use_introns_from_gtf(false)
	{}

	RefGenesContainerWithIntrons::RefGenesContainerWithIntrons(const std::string &genes_filename)
		: _is_empty(false)
		, _use_introns_from_gtf(false)
	{
	...
	void RefGenesContainerWithIntrons::init(const std::string &genes_filename)
	{
		...
		while (std::getline(gz_fs, line))
		{
			...
			try
			{
				...
				{
					record = RefGenesContainerWithIntrons::parse_gtf_record(line);
				}
				else
				{
					record = RefGenesContainerWithIntrons::parse_bed_record(line);
				}
			}
		...
		for (auto const &chr : this->_transcript_positions)
		{
			...
			{
				intervals.add_interval(transcript.second.first, transcript.second.second, transcript.first);
	...
	void RefGenesContainerWithIntrons::save_transcript(GtfRecord record)
	{
		auto transcript_iter = this->_transcript_positions[record.chr_name()].emplace(record.transcript_id(), std::make_pair(record.start_pos(), record.end_pos()));
		transcript_iter.first->second.first = std::min(transcript_iter.first->second.first, record.start_pos());
		transcript_iter.first->second.second = std::max(transcript_iter.first->second.second, record.end_pos());
	...
	GtfRecord RefGenesContainerWithIntrons::parse_gtf_record(const std::string &record)
	{
		GtfRecord result;
		if (record.at(0) == '#')
			return result;

		std::vector<std::string> columns(RefGenesContainerWithIntrons::split(record));
	...
	RefGenesContainerWithIntrons::query_results_t RefGenesContainerWithIntrons::get_gene_info(const std::string &chr_name, pos_t start_pos, pos_t end_pos) const
	{
	...
	GtfRecord RefGenesContainerWithIntrons::parse_bed_record(const std::string &record)
	{
		GtfRecord result;
	...
	std::vector<std::string> RefGenesContainerWithIntrons::split(const std::string &record)
	{
	...
	bool RefGenesContainerWithIntrons::is_empty() const
	{
		return this->_is_empty;
	}

	bool RefGenesContainerWithIntrons::QueryResult::operator<(const QueryResult &other) const
	{
	...
	RefGenesContainerWithIntrons::QueryResult::QueryResult(const std::string &gene_name, GtfRecord::RecordType type)
		: gene_name(gene_name)
		, type(type)
	{}
}
