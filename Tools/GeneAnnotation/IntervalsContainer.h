	...
	namespace GeneAnnotation
	{
				...
				for (auto const &event : events)
				{
					...
					if (!cur_labels.empty() && end_pos - start_pos >= this->_min_interval_len)
					{
						if (!this->_allow_intercepts && cur_labels.size() > 1)
							throw std::runtime_error("Intervals intersection at (" + std::to_string(start_pos) + ", " +
		...
		public:
			...
			interval_labels_t get_intervals(coord_t start_pos, coord_t end_pos) const
			{
				...
				auto intercept_it = std::lower_bound(this->_homogenous_intervals.begin(),
				                                     this->_homogenous_intervals.end(), start_pos,
				                                     [](const QueryInterval &interval, coord_t pos) {
					                                     return interval.end_pos() <= pos;
				                                     });
				...
				{
					res_labels.insert(intercept_it->base_interval_labels.begin(),
					                  intercept_it->base_interval_labels.end());
					++intercept_it;
				}
