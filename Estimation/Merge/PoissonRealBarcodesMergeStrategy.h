...
namespace Merge
{
	...
	private:
		double get_bootstrap_intersect_prob(const genes_t &cell1_dist, const genes_t &cell2_dist,
											size_t intersect_size, size_t fit_size = 3000, unsigned multiplies_count = 4);
	...
	public:
		...
		double estimate_by_r(ul_list_t vector, size_t i);

		double get_bootstrap_intersect_sizes(const genes_t &cell1_dist, const genes_t &cell2_dist,
											 size_t real_intersect_size, size_t repeats_count, ul_list_t sizes);
	};
}
}