...
SpacerFinder::SpacerFinder(const boost::property_tree::ptree &config)
{
	...
	if (this->spacer.length() <= SpacerFinder::spacer_prefix_length)
		throw std::runtime_error("Spacers length should be bigger then spacer_prefix_length ('" + spacer + "): '" + spacer + "'");
}
...
{
	return seq.substr(spacer_pos + this->spacer.length() + this->barcode_length + this->umi_length - this->r1_rc_length, this->r1_rc_length);
}
