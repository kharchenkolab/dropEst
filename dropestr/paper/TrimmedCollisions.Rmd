...
ggsave(fig_collisions, filename=paste0(kPlotsFolder, '/FigureCollisons.pdf'), height=5.5, width=3.5)
fig_collisions
```

```{r, fig.width=4, fig.height=5}
gg_collisions_ext <- ggplot(mapping=aes(x=Observed, col=as.factor(UmiLength), y=Adjusted / AdjustedClassic)) + 
  geom_smooth(data=df_s$Direct, mapping=aes(linetype='Empirical, back trim'), size=0.9, se=F) +
  geom_smooth(data=df_s$Reverse, mapping=aes(linetype='Empirical, front trim'), size=0.9, se=F) +
...
distrs_by_length <- mclapply(prefix_lengths, MaxFreqDistribution, umi_distribution_tail, mc.cores=10)
plot_df <- lapply(1:length(prefix_lengths),
                  function(i) cbind(melt(distrs_by_length[[i]], id.vars = 'fracs'), PrefixLength=prefix_lengths[i]))
...
gg_stretches <- ggplot(plot_df, aes(x = fracs, y = as.factor(PrefixLength), height = value,
  ...
  guides(fill=gl, linetype=gl) + scale_fill_npg()
...
ggsave(gg_figure, filename=paste0(kPlotsFolder, 'CollisionsSupFig.pdf'), width=7, height=8)
gg_figure
```
