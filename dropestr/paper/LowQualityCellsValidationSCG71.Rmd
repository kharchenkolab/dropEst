...
unclassified_cbs <- setdiff(colnames(r_cm_rescued), names(clusters))
clusters[unclassified_cbs] <- apply(r_rescued$reductions$PCA[unclassified_cbs, , drop=F], 1, function(cell)
  sapply(cluster_centers, cor, cell) %>% which.max() %>% names())
...
```{r}
srt@ident <- as.factor(clusters[colnames(srt@raw.data)])
...
ggs_annot <- lapply(ggs, function(gg) cowplot::plot_grid(plotlist=lapply(gg, `+`, theme(legend.position="none", plot.margin=margin())), nrow=1, rel_widths=c(1.5, 0.1, 0.1), align='h'))
