...
```{r}
ReadsPerUmiDataset <- function(reads.per.umi.per.cb, max.umis.per.cb=4) {
...
```{r}
# rpu_distribution[1:r_max] / sum(rpu_distribution[1:r_max])
classifier <- list(ReadsPerUmi=rpu_distribution, ReadsPerUmiNegTheta=theta, ReadErrorProb=read_error_prob)
```

```{r}
classifier_df <- data.frame(Base=names(trimmed$umi.probabilities)[1:10], Target='AAAAAA', MinRpU=1, MaxRpU=44)
```
```{r}
rpus_filt <- mclapply(trimmed$correction.info$rpus.with.inds, FilterUmisInGeneNew, trimmed$umi.probabilities, 
                      probs_given_reads_large, classifier, mc.cores=30)

rpus_filt <- lapply(trimmed$correction.info$rpus.with.inds[1:100], FilterUmisInGeneNew, trimmed$umi.probabilities, 
                      probs_given_reads_large, classifier)
...
```{r}
# trimmed$correction.info$rpus.with.inds
# trimmed$correction.info$neighbours.per.umi

# (cur_gene <- ExtractReadsPerUmi(trimmed$reads.per.umi.per.cb[trimmed$umis.per.gene > 3][[3]], one.gene=T))
(cur_gene <- trimmed$reads.per.umi.per.cb[trimmed$umis.per.gene > 3][[3]]) # [[3]]

cur_neighborhood <- SubsetAdjacentUmis(names(cur_gene))
classifier_df <- PrepareClassifierData(cur_gene, cur_neighborhood, trimmed$umi.probabilities)
classifier_df <- classifier_df[order(classifier_df$Target, classifier_df$Base),]

filtered_mask <- classifier_df %>% group_by(Target, MaxRpU) %>% 
  mutate(IsMerged=1:length(Base) <= (apply(as.matrix(probs_given_reads_large[, MaxRpU]), 2, which.max) - 1)) %>%
  .$IsMerged

if (any(filtered_mask)) {
  filt_predictions <- classifier_df[filtered_mask,]
  filtered_mask[which(filtered_mask)] <- ResolveUmisDependencies(as.character(classifier_df$Base),
                                                                 as.character(classifier_df$Target),
                                                                 classifier_df$MaxRpU)
}
```
