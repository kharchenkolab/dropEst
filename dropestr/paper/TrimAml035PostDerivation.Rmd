...
```{r}
library(Rcpp)
# tprobs <- setNames(1:100, paste0(1:100))
umi_proobabilities_map <- new(CppMap, names(trimmed$umi.probabilities), as.vector(trimmed$umi.probabilities))
```

```{r}
rpus_filt <- mclapply(trimmed$reads.per.umi.per.cb, FilterUmisInGeneNew, umi_proobabilities_map, 
                      classifier, mc.cores=30)

rpus_filt <- lapply(trimmed$reads.per.umi.per.cb[1:1000], FilterUmisInGeneNew, umi_proobabilities_map, 
                    classifier)

r0 <- FilterUmisInGeneNew(trimmed$reads.per.umi.per.cb[trimmed$umis.per.gene == 50][[2]], umi_proobabilities_map, classifier)
