...
# ggplot(melt(umis_per_gene, id.vars='raw')) +
#   geom_point_rast(aes(x = raw, y = raw - value, color = variable, size = 1, alpha=0.05)) +
...
We can see that a lot of clusters has the same set of highly-expressed genes. Let's choose some such genes and clusters.
...
ggplot(plot_df) + 
  ...
  labs(x = 'Edit distance', y = 'Relative error, %') +
  theme_pdf(legend.pos=c(1, 1)) +
  theme(panel.grid.major=element_blank()) + scale_y_continuous(expand=c(0.0, 0))
```
