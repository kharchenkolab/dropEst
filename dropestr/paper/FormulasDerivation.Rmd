...
```{r}
estimated_nn <- EstimatedNumberOfAdjacentUmisByGeneSize(trimmed$correction.info$dp.matrices, 
                                                        trimmed$correction.info$neighb.prob.index, trimmed$umi.probabilities)
umis_per_gene_est <- 2:length(estimated_nn)
estimated_nn <- estimated_nn[umis_per_gene_est]
```
...
qplot(x=umis_per_gene_est, y=estimated_nn, geom='line', color='Estimated') + 
...
Current estimation uses adjusted number of UMIs, while observed number isn't adjusted. Let's perform collisions adjustment:
```{r}
umis_per_gene_uniq_adj <- sapply(umis_per_gene_uniq, AdjustGeneExpression, trimmed$collisions.info$adjusted, trimmed$collisions.info$observed)
qplot(x=umis_per_gene_est, y=estimated_nn, geom='line', color='Estimated') + 
  geom_point(aes(x=umis_per_gene_uniq_adj, y=mean_neighbours_num, color='Observed'), size=0.5) + 
  labs(x='#UMIs per gene', y='Mean #adjacent UMIs') + scale_color_npg(name='') + scale_x_log10()

qplot(x=umis_per_gene_est, y=estimated_nn / (umis_per_gene_est - 1), geom='line', color='Estimated') + 
  geom_point(aes(x=umis_per_gene_uniq_adj, y=mean_neighbours_num / (umis_per_gene_uniq_adj - 1), color='Observed'), size=0.5) + 
...
```{r}
err_by_size <- ErrorByGeneSize(trimmed$correction.info$dp.matrices, trimmed$correction.info$neighb.prob.index, 
                               trimmed$umi.probabilities, 3*6, 0.46)

qplot(x=1:length(err_by_size), y=err_by_size + c(0, estimated_nn / (umis_per_gene_est - 1)), geom='line', color='Estimated') + 
  geom_point(aes(x=umis_per_gene_uniq_adj, y=mean_neighbours_num / (umis_per_gene_uniq_adj - 1), color='Observed'), size=0.5) + 
  labs(x='#UMIs per gene', y='Probability of a pair of adjacent UMIs') + scale_color_npg(name='') +
  scale_x_log10() + scale_y_log10()
```
