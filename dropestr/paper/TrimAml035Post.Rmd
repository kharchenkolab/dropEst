...
trimmed$filt_cells$NB_p005 <- CorrectUmiSequenceErrors(trimmed_reads_per_umi_per_cb, umi.probabilities=trimmed$umi.probabilities,
                                                   collisions.info=trimmed$collisions.info, correction.info=trimmed$correction.info,
                                                   mc.cores=30, return='umis', verbosity.level=2, error.prior.prob=0.005)

trimmed$filt_cells$NB2 <- CorrectUmiSequenceErrors(trimmed_reads_per_umi_per_cb, umi.probabilities=trimmed$umi.probabilities,
                                                   collisions.info=trimmed$collisions.info, correction.info=trimmed$correction.info,
                                                   mc.cores=30, return='umis', verbosity.level=2)
```
...
ggplot(p0 %>% head(1000), aes(x=as.factor(ceiling(RealValue / 5) * 5) , y=100 * (value - RealValue) / RealValue, color=Correction)) + 
...
# New ideas
```{r}
reads_per_umi_trimmed <- lapply(trimmed$reads.per.umi.per.cb, sapply, `[[`, 1)
...
PlotReadsDistribution <- function(reads.per.gene, umis.per.gene, min.umis=NULL, max.umis=NULL) {
  ...
  ggplot() + geom_histogram(aes(x=rpus.filt, y=..count.. / sum(..count..)), bins=50) + 
    xlim(0, 20) + labs(title=paste0("Min: ", min.umis, ", max: ", max.umis), x='Reads per UMI', y='#UMIs')
}
